...
public class ValidatorPessoa extends Pessoa {
	// VALIDATE NAME
	...
	public static void findExistentCod(ArrayList<Pessoa> list1, Integer FEcod) throws InvalidCodeException {
		...
		while (iterator.hasNext()) {
			...
			if (p instanceof Cliente) {
				...
				if (((Integer) c.getCodCliente()).equals(FEcod)) {
					throw new InvalidCodeException(
							"Esse código já está registrado, por favor, utilize outro código. -> Código digitado: ",
							FEcod);
				}
			} else if (p instanceof Funcionario) {
				Funcionario f = (Funcionario) p;

				if (((Integer) f.getIdEmployee()).equals(FEcod)) {
					throw new InvalidCodeException(
							"Esse ID já está registrado, por favor, utilize outro ID. -> ID digitado: ", FEcod);
				}
			}
		}
	}
